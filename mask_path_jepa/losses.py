...
def gated_centroid_nn_loss(
    ...
    *,
    ...
    only if cosine similarity >= ``tau`` (and optionally mutual top-k). If
    ``sim_gap`` > 0, also require top1_sim - top2_sim >= sim_gap (clear nearest).
    If no pair clears the gate, the loss is 0.
    """
    if centroids.ndim != 2:
    ...
    # Need top-2 when applying a uniqueness gap on the nearest neighbor.
    n_retrieve = kk if float(sim_gap) <= 0.0 else min(max(kk, 2), b - 1)
    top_sim_full, top_idx_full = sim.topk(n_retrieve, dim=1)
    top_sim = top_sim_full[:, :kk]
    top_idx = top_idx_full[:, :kk]
    gate = top_sim >= float(tau)

    if float(sim_gap) > 0.0 and n_retrieve >= 2:
        # Clear nearest: only gate the first neighbor if it beats 2nd by sim_gap.
        gap_ok = (top_sim_full[:, 0] - top_sim_full[:, 1]) >= float(sim_gap)
        gate = gate.clone()
        gate[:, 0] = gate[:, 0] & gap_ok
        if kk > 1:
            # Extra neighbors (rare): also require beating the next retrieved sim.
            for t in range(1, kk):
                if t + 1 < n_retrieve:
                    gate[:, t] = gate[:, t] & (
                        (top_sim_full[:, t] - top_sim_full[:, t + 1]) >= float(sim_gap)
                    )
    ...
    if mutual:
        ...
        j = top_idx
        nbr_of_j = top_idx[j]
        i_in_j = (nbr_of_j == eye.unsqueeze(-1)).any(dim=-1)
        gate = gate & i_in_j
...
def intra_centroid_pull_loss(
    z_views: torch.Tensor,
    *,
    stopgrad_centroid: bool = True,
) -> tuple[torch.Tensor, dict[str, float]]:
    """Pull the M aug embeddings of each image toward their centroid.

    ``z_views``: (B, M, D). Centroid ``c_i = normalize(mean_m z_i^m)``.
    Loss: mean_{i,m} (1 - cos(z_i^m, c_i)).

    If ``stopgrad_centroid``, gradients flow only through the views (classic
    multi-view align-to-mean). Same-image term; complements inter-image
    pull/push on centroids.
    """
    if z_views.ndim != 3:
        raise ValueError(f"z_views must be (B, M, D), got {tuple(z_views.shape)}")
    b, m, _d = z_views.shape
    stats = {"intra_mean_sim": 0.0, "intra_views": float(b * m)}
    if m < 2:
        return z_views.new_zeros(()), stats

    z = F.normalize(z_views, dim=-1)
    c = F.normalize(z.mean(dim=1), dim=-1)  # (B, D)
    if stopgrad_centroid:
        c = c.detach()
    # (B, M) cosines of each view to its image centroid
    sim = (z * c.unsqueeze(1)).sum(dim=-1)
    loss = (1.0 - sim).mean()
    with torch.no_grad():
        stats["intra_mean_sim"] = float(sim.mean().item())
    return loss, stats


def pull_push_centroid_loss(
    centroids: torch.Tensor,
    *,
    push_weight: float = 1.0,
    mutual_pull: bool = False,
) -> tuple[torch.Tensor, dict[str, float]]:
    """Hard nearest pull + farthest push on centroid cosine distances (sole SSL idea).

    For each i (j≠i):
      j* = argmax sim_ij   → L_pull = 1 - sim_{i,j*}
      k* = argmin sim_ij   → L_push = -(1 - sim_{i,k*})   # maximize distance to farthest

    L = mean(L_pull) + push_weight * mean(L_push)

    If ``mutual_pull``, only apply pull when i is also j*'s nearest (else pull=0 for that i).
    No other regularizers — result is attributable to this term alone.
    """
    if centroids.ndim != 2:
        raise ValueError(f"centroids must be (B, D), got {tuple(centroids.shape)}")
    b = centroids.size(0)
    ...
    c = F.normalize(centroids, dim=-1)
    sim = c @ c.T
    eye = torch.eye(b, device=centroids.device, dtype=torch.bool)
    sim_pos = sim.masked_fill(eye, float("-inf"))
    sim_neg = sim.masked_fill(eye, float("inf"))

    near_sim, near_idx = sim_pos.max(dim=1)
    far_sim, _far_idx = sim_neg.min(dim=1)

    pull = 1.0 - near_sim
    if mutual_pull:
        # i pulls j* only if i is also nearest of j*
        partner_near = near_idx[near_idx]
        i_ids = torch.arange(b, device=centroids.device)
        mutual = partner_near == i_ids
        pull = pull * mutual.float()
        stats["nn_pairs"] = float(mutual.sum().item())
        stats["nn_frac_gated"] = float(mutual.float().mean().item())

    push = -(1.0 - far_sim)  # more negative when far_sim is small (good separation)
    lam = float(push_weight)
    loss = pull.mean() + lam * push.mean()

    with torch.no_grad():
        stats["nn_pull"] = float(pull.mean().item())
        stats["nn_push"] = float(push.mean().item())
        stats["nn_near_sim"] = float(near_sim.mean().item())
        stats["nn_far_sim"] = float(far_sim.mean().item())
        stats["nn_mean_sim"] = stats["nn_near_sim"]
    return loss, stats


def softmax_centroid_nn_loss(
    centroids: torch.Tensor,
    *,
    temperature: float = 0.05,
    mutual: bool = True,
) -> tuple[torch.Tensor, dict[str, float]]:
    """Soft neighbor InfoNCE on centroid similarities (continuous in-batch NN).

    For each i, ``p_ij = softmax_j(sim_ij / T)`` with j≠i. Stop-grad targets
    ``w`` (optionally symmetrized) so the model matches current soft neighbors
    via cross-entropy — attraction to likely neighbors **and** repulsion from
    the rest through the softmax partition:

        L = mean_i [ - sum_j sg(w_ij) log p_ij ]

    Note: ``L = sum w(1-sim)`` is *not* used — that objective is minimized by
    total collapse (sim→1 ⇒ L→0). Soft CE stays large under collapse (~ln(B-1))
    but still needs an explicit anti-collapse term (e.g. uniformity) to leave
    the flat all-equal critical point.
    """
    if centroids.ndim != 2:
        raise ValueError(f"centroids must be (B, D), got {tuple(centroids.shape)}")
    b = centroids.size(0)
    stats = {
        "nn_pairs": float(b),  # every image contributes
        "nn_mean_sim": 0.0,
        "nn_frac_gated": 1.0,
        "nn_entropy": 0.0,
        "nn_max_p": 0.0,
    ...
    logits = logits.masked_fill(
        torch.eye(b, device=centroids.device, dtype=torch.bool),
        float("-inf"),
    )
    ...
    # Soft CE: pull toward sg(w) neighbors, push others via Z(softmax).
    # Mask diagonal: log_p_ii = -inf from the eye fill; 0*(-inf) → NaN.
    eye = torch.eye(b, device=centroids.device, dtype=torch.bool)
    loss = -(w.detach() * log_p).masked_fill(eye, 0.0).sum(dim=1).mean()

    with torch.no_grad():
        p_safe = p.clamp_min(1e-12)
        entropy = -(p_safe * p_safe.log()).sum(dim=1).mean()
        max_p = p.max(dim=1).values.mean()
        mean_sim = (p * sim).sum(dim=1).mean()
        stats["nn_entropy"] = float(entropy.item())
        stats["nn_max_p"] = float(max_p.item())
        stats["nn_mean_sim"] = float(mean_sim.item())
    return loss, stats
...
def active_nn_tau(
    ...
    """Cosine-similarity threshold curriculum (high → lower), fast early drop.

    If ``anneal_epochs`` is set, τ reaches ``tau_end`` by that epoch and stays
    there. Uses a square-root schedule so most of the drop happens at the start.
    """
    horizon = int(anneal_epochs) if anneal_epochs is not None else int(epochs)
    horizon = max(horizon, 1)
    if horizon <= 1:
        return float(tau_end)
    # epoch 1 → t=0, epoch==horizon → t=1
    t = min(max(epoch - 1, 0), horizon - 1) / float(horizon - 1)
    # sqrt: steeper early decrease than linear
    t_fast = t**0.5
    return float(tau_start + t_fast * (tau_end - tau_start))
