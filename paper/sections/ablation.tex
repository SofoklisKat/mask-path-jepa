...
\begin{table*}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    SwAV                            & --- & --- & --- & --- \\
    \midrule
    \textbf{Ours: InfoNCE + MVC}    & \textbf{0.193} & 0.154 & --- & --- \\
    \textbf{Ours: InfoNCE + SIGReg} & 0.179 & \textbf{0.175} & --- & --- \\
    \textbf{Ours: InfoNCE + SIGReg (R50)} & 0.095$^\S$ & 0.115$^\S$ & --- & --- \\
    \bottomrule
  \multicolumn{5}{l}{\footnotesize $^*$@20 epochs; $^\S$@62 epochs, ResNet-50 scratch, batch 128.}
  \end{tabular}
\end{table*}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/ablation_scatter.png}
  \caption{k-NN vs.\ linear probe trade-off on CIFAR-100 (SmallResNet unless noted).
  Stars mark the final evaluated epoch per run; curves connect 10-epoch evaluations.}
  \label{fig:tradeoff}
\end{figure}

\paragraph{Learning curves (same network).}
Figure~\ref{fig:curves} shows epoch-wise k-NN and linear accuracy for completed runs.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/ablation_curves.png}
  \caption{Training curves at 10-epoch intervals.
  Left: k-NN@20; right: linear probe.
  Solid lines: SmallResNet; extend with additional runs as the 200-epoch ablation completes.}
  \label{fig:curves}
\end{figure}
